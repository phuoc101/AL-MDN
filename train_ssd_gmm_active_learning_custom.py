...
parser.add_argument(
    "--num_workers", default=8, type=int, help="Number of workers used in dataloading"
)
parser.add_argument(
...
parser.add_argument(
    "--eval_save_folder", default="eval/", type=str, help="File path to save results"
)
parser.add_argument(
    "--use_cuda", default=True, help="if True use GPU, otherwise use CPU"
)
parser.add_argument("--id", default=1, type=int, help="the id of the experiment")
parser.add_argument(
    "--top_k",
    default=5,
    type=int,
...
def create_loaders():
    ...
    if cfg["name"] == "VOC":
        supervised_dataset = VOCDetection(
            root=args.voc_root, transform=SSDAugmentation(cfg["min_dim"], MEANS)
        )
    ...
    if cfg["name"] == "VOC_picam":
        ...
        supervised_dataset = COCODetection(
            root=args.coco_root, transform=SSDAugmentation(cfg["min_dim"], MEANS)
        )
        unsupervised_dataset = COCODetection(
            args.coco_root, transform=BaseTransform(300, MEANS)
        )

    supervised_data_loader = data.DataLoader(
        supervised_dataset,
    ...
    unsupervised_data_loader = data.DataLoader(
    ...
    while finish_flag:
        ...
        for iteration in range(args.start_iter, cfg["max_iter"]):
            ...
            if iteration % 10 == 0:
                ...
                print(
                    "loss: %.4f , loss_c: %.4f , loss_l: %.4f , lr : %.4f\n"
                    % (
                        loss.data,
                        loss_c.data,
                        loss_l.data,
...
def main():
    ...
    if cfg["name"] == "VOC":
        ...
        # )
        # print(f"Evaluation: mAP: {mean_ap}")

        # active learning
        net = build_ssd_gmm("train", cfg["min_dim"], cfg["num_classes"])
        net = nn.DataParallel(net)
        print("loading weight {}...".format(weight_name))
        net.load_state_dict(torch.load(weight_name))

        net.eval()
        batch_iterator = iter(unsupervised_data_loader)
        labeled_set, unlabeled_set, to_label_set = active_learning_cycle(
            batch_iterator,
            labeled_set,
            unlabeled_set,
            net,
        ...
        print(
            ...
            f"To Label: {len(to_label_set)}",
        )

        # save the labeled training set list
        f = open(
            "labeled_training_set_"
            + cfg["name"]
            + "_"
            + str(len(labeled_set))
            + "_id_"
        ...
        for i in range(len(labeled_set)):
            ...
            f.write("\n")
        f.close()
        # save to label set list
        f = open(
            "to_label_training_set_"
            + cfg["name"]
            + "_"
            + str(len(to_label_set))
            + "_id_"
            + str(args.id)
            + ".txt",
            "w",
