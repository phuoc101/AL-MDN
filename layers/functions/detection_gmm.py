...
class Detect_GMM(Function):
    ...
    def forward(
        ...
        output = torch.zeros(num, ctx.num_classes, ctx.top_k, 15)
        ...
        new_conf = new_conf.view(num, num_priors, ctx.num_classes).transpose(2, 1)
        cls_al_uc = cls_al_uc.view(num, num_priors, ctx.num_classes).transpose(2, 1)
        cls_ep_uc = cls_ep_uc.view(num, num_priors, ctx.num_classes).transpose(2, 1)

        # Decode predictions into bboxes.
        for i in range(num):
            decoded_boxes = decode(new_loc[i], prior_data, ctx.variance)
